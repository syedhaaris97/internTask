...
<script type="text/javascript">
	...
	function putMeIn(ref, i, j) {
		alert(i + " - " + j);
		// var ds = document.getElementById("aaa");
		// voluneers[i][j]=ref.cloneNode(false);
		console.log(ref.cloneNode(false));
		// var ds = document.getElementById("aaa").appendChild(ref.cloneNode(false));
        ...
        for (var i = 0; i < 10; i++) {
            ...
            for (var j = 0; j < 10; j++) {
                ...
                cell.appendChild(ref.cloneNode(false));
            }
        } 
	...
	function prepareGird() {
		for (var i = 0; i < voluneers.length; i++) {
			...
			alert(voluneers[i][j]) = (voluneers[i][j] == undefined)? x.cloneNode(false):voluneers[i][j];
			}
		}
	}
